...
pub fn prove_verify_incremental_one<'a>(
    ...
    let trace_constant = vec![trace_0[1].clone(), trace_1[1].clone()];
    let trace_witness = vec![trace_0[0].clone(), trace_1[0].clone()];

     // Preprocessed columns.

     let mut tree_builder = commitment_scheme.tree_builder();
     let constants_trace_location = tree_builder.extend_evals(trace_constant);
     tree_builder.commit(channel);
     //channel.mix_u64(log_n_rows as u64);
     //channel.mix_u64((log_n_rows-1) as u64);

    let mut tree_builder = commitment_scheme.tree_builder();
    let base_trace_location = tree_builder.extend_evals(trace_witness);
    tree_builder.commit(channel);
...
mod tests {
    ...
    use crate::examples::incre_one_multi_fix::prove_verify_incremental_one;
